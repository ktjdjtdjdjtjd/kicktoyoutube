...
- ダンマクは**ストリップ方式**: テキスト(白・BIZ UDPGothic・黒縁)とエモート画像(フルカラー)を
  ...
  速度はレーンごとに散らすので、画面上は速いコメと遅いコメが混在して追い越しが起きる。
  倍率は `speed` の 1±`danmaku.speed_spread`(既定0.35)を14レーンぶん等間隔に刻み、
  **VOD ID由来のseedで割り当てるため配信ごとに並びが変わる**。等間隔＝平均倍率はちょうど1.0
  なのでストリップ総幅(=ffmpegの常駐メモリ)は等速時と同じ。`speed_spread: 0` で全て等速に戻る。
  倍率を自分で決めたいときは `danmaku.speed_variants`(倍率リスト・レーンへ巡回割当)を書けばそちらが優先。
