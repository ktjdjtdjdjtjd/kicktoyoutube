...
export async function handle(request,env,html=''){
  const url=new URL(request.url),path=url.pathname,ingest=['/ingest/','/ingest-edit/','/ingest-feedback/'].some(prefix=>path.startsWith(prefix));
  ...
  try{
    const review=await reviewRoute(request,env);if(review)return review;
